...
public class BufMgr implements GlobalConst{
  ...
  /** 
   * Create a buffer manager object. We add an argument to the previous constructor : lastRef
   *
  ...
  public BufMgr( int numbufs, String replacerArg, int lastRef)
	...
	frmeTable[i] = new FrameDesc();
	  ...
	  {
 	/** 
	We use MRU (and not LRU as mentioned before) when MRU is called
   	*/
	...
	/**
	* We use LRUK when LRUK is called and add lastRef as argument
        * If lastRef = 1, we use LRU as LRU-1 = LRU
   	*/
	else if(replacerArg.compareTo("LRUK")==0)
	  {
	   if (lastRef == 1) {
		replacer = new LRU(this);
	   	System.out.println("Replacer: LRU-" + lastRef + " ie LRU\n");
          } else {
	    replacer = new LRUK(this);
	    System.out.println("Replacer: LRU-" + lastRef + "\n");
	  }
	}
	else
	  {
  ...
  /** Gets the replacer of the buffer.
  *
  * @return the replacer of the buffer.
  */
 public Replacer getReplacer() { return replacer; }

    /** Gets the last reference k.
   *
   * @return last reference k.
   */
