 ...
 */
class BMDriver2 extends TestDriver implements GlobalConst {
 ...
 /**
 * We initialize the lastRef value now
 *
 */ 
  private int k = 2; 
  
  /**
   * BMDriver2 Constructor, inherited from TestDriver
   */
  public BMDriver2 () {
    super("buftest");
  }
  ...
  protected boolean test1 () {
    ...
    for ( pid.pid = firstPid.pid, lastPid.pid = pid.pid+numPages-1; 
      ...
      try {
    //System.out.println("PID NUMERO:" + pid);
  ...
  protected boolean test2 () {
	...
	/****CALL THE PICK_VICTIM METHOD... DISPLAY THE PAGE ID THAT HAS BEEN CHOOSEN AS VICTIM*****/
      status = FAIL;
...
SystemDefs sysdef = new SystemDefs( dbpath, NUMBUF+20, NUMBUF, replacealgo[i], k);
  ...
  protected boolean test4 () {
	    ...
	    boolean status = OK;
            ...
            /**
            * We add lastRef as an argument of the initialization of the system defs
            *
            */ 

	    SystemDefs sysdef = new SystemDefs( dbpath, NUMBUF+20, NUMBUF, "LRUK", k);
	    
	    if(!(SystemDefs.JavabaseBM.getReplacer() instanceof LRUK)) return true;
...
public class BMTest2 {

   public static void main (String argv[]) {

     BMDriver2 bmt = new BMDriver2();
     boolean dbstatus;
